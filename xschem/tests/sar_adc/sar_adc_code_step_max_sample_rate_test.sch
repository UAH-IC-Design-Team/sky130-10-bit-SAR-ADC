...
S {}
E {}
T {This reset is active LOW!!!} 720 -180 0 0 0.4 0.4 {}
N 50 -250 50 -230 {
lab=GND}
N 260 -250 260 -230 {
lab=GND}
...
while vdiff le $&vmax
	...
	* Measure the max to find the output
	meas tran max_bit0 MAX v(bits1) from=200n to=205n
	meas tran max_bit1 MAX v(bits2) from=200n to=205n
	meas tran max_bit2 MAX v(bits3) from=200n to=205n
	meas tran max_bit3 MAX v(bits4) from=200n to=205n
	meas tran max_bit4 MAX v(bits5) from=200n to=205n
	meas tran max_bit5 MAX v(bits6) from=200n to=205n
	meas tran max_bit6 MAX v(bits7) from=200n to=205n
	meas tran max_bit7 MAX v(bits8) from=200n to=205n
	meas tran max_bit8 MAX v(bits9) from=200n to=205n
	meas tran max_bit9 MAX v(bits10) from=200n to=205n
	meas tran avg_vsampled_p AVG v(x1.vsampled_p) from=190n to=195n
	meas tran avg_vsampled_n AVG v(x1.vsampled_n) from=190n to=195n
...
.endc
"}
C {devices/vsource.sym} 620 -120 0 0 {name=V5 value="PULSE 0 1.8V 500ns 0.1ns 0.1ns 320ns 640ns"
}
C {devices/gnd.sym} 620 -70 0 0 {name=l18 lab=GND}
C {devices/lab_pin.sym} 620 -150 0 0 {name=l19 sig_type=std_logic lab=clk
}
C {devices/vsource.sym} 890 -120 0 0 {name=V6 value="PULSE 0 1.8V 500ns 0.1ns 0.1ns 1s 1s"
}
C {devices/gnd.sym} 890 -70 0 0 {name=l20 lab=GND}
C {devices/lab_pin.sym} 890 -150 0 0 {name=l21 sig_type=std_logic lab=reset_b
}
